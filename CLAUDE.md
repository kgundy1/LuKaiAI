...
- Include "Generated by Claude Code" attribution in PR descriptions, PR titles, commit messages, or commit bodies
...
### Git identity — required check before every Code commit

Every Code session must set the local git identity to the repo owner BEFORE making any commit. Code's default identity is `Claude <noreply@anthropic.com>`, which violates the project's no-AI-attribution policy. The required pre-commit setup:

```
git config user.name "kgundy1"
git config user.email "kgundy1@gmail.com"
```

Verify with `git config user.name && git config user.email` before the first commit and again before any commit that follows a session restart. If a commit has already been made with the wrong identity, fix it with `git commit --amend --reset-author --no-edit` and force-push to the feature branch (never to main).

### PR body and commit message — no AI attribution

Code's default behavior is to append a "Generated by Claude Code" footer with a session URL to PR descriptions and to add a "Co-authored-by: Claude <noreply@anthropic.com>" trailer to commit messages. Both are forbidden. Every Code prompt must explicitly say: *No attribution, no Co-authored-by, no claude.ai session URLs in PR title, body, or commit message.* Every PR review must audit the PR body AND the commit message for these strings before merge.
...
**Canonical source:** For any visual or brand decision, refer to `design-system/README.md` — that's the authoritative spec for colors, typography, voice, tone, motion, layout, cards, forms, and iconography. The rules below in this CLAUDE.md are a summary; the design system is the full version. If they conflict, the design system wins. The design system is also available as a Claude Skill via `design-system/SKILL.md` for future sessions.
...
The Cloudflare project is configured to build apps/web/ (the React app) and publish apps/web/dist/. The root-level index.html (the rich landing page) is NOT in that output by default — so without an explicit copy step, Cloudflare ships the React SPA's tiny shell index.html instead and the real landing page never reaches production. The fix is currently in the Cloudflare dashboard build command: `cd apps/web && npm install && npm run build && cp ../../index.html dist/index.html`. This needs to move to a postbuild script in apps/web/package.json so the fix is in code, not platform config. Filed in Phase 5b.
