...
4. **🟡 PARTIALLY DONE — Adopt interactive widgets one at a time.** Live in lessons today: **QuickCheck** (3 lessons) and **WorkflowSorter** (2 lessons). Not yet adopted by any live lesson: **PromptCompare**, **DecisionTree**, **TryWithClaude**. Note: PromptCompare and DecisionTree were drafted in the original JSX kit as one-off demos with hardcoded content. They have no payload-driven mode yet. When a lesson actually wants to use one, it'll need to be refactored to accept payload props (the way QuickCheck and WorkflowSorter already do). This is the time to do that — when there's a real lesson driving the requirements.
